...
class HashTable {
    ...
    public HashTable(int size_param){
    ...
    public void insert(Personagem personagem){
        ...
        if (countComp++ >= 0 && hash_table[key] == null) {
            ...
            int rehash_key = rehash(personagem.getName());
            if (countComp++ >= 0 && hash_table[rehash_key] == null) {
                hash_table[rehash_key] = personagem;
            }
        }
    }
    ...
    public int search(String name){
        ...
        if (countComp++ >= 0 && hash_table[key] == null) {
            ...
            int rehash_key = rehash(name);
            if (countComp++ >= 0 && hash_table[rehash_key] == null) {
                resp = -1;
            }
            else if (countComp++ >= 0 && hash_table[rehash_key].getName().equals(name)) {
                resp = rehash_key;
            }
        }
        return resp;
...
class Personagem implements Comparable<Personagem> {
    ...
    public void read(String line) throws Exception {
        String[] info = TP04Q06.cutter(';', line);

        setId(info[0]);
        setName(info[1]);

        // alternate names
        String tmp = TP04Q06.shambles("[", "", info[2]);
        tmp = TP04Q06.shambles("]", "", tmp);
        String[] alternateNamesArray = TP04Q06.cutter(',', tmp);

        for (int i = 0; i < alternateNamesArray.length; i++) {
            String alternateSTR = alternateNamesArray[i].trim();
            alternateSTR = TP04Q06.shambles("'", "", alternateSTR);
        ...
        // alternate actors
        String tmp2 = TP04Q06.shambles("[", "", info[11]);
        tmp2 = TP04Q06.shambles("]", "", tmp2);
        String[] alternateActorsArray = TP04Q06.cutter(',', tmp2);
    ...
    //         String[] dateCut = TP04Q06.cutter('-', date);
    //         date = dateCut[2];
    //         date += "-";
    //         date += dateCut[1];
    //         date += "-";
...
public class TP04Q06{
    ...
    public static void main(String[] args) throws Exception {
        ...
        String line = "";
        HashTable characters = new HashTable(21);
        ...
        try (BufferedWriter BW = new BufferedWriter(new FileWriter("matricula_hashRehash.txt"))) {
