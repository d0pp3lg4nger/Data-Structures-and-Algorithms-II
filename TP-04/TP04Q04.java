...
class Personagem implements Comparable<Personagem> {
    ...
    public void read(String line) throws Exception {
        String[] info = TP04Q04.cutter(';', line);

        setId(info[0]);
        setName(info[1]);

        // alternate names
        String tmp = TP04Q04.shambles("[", "", info[2]);
        tmp = TP04Q04.shambles("]", "", tmp);
        String[] alternateNamesArray = TP04Q04.cutter(',', tmp);

        for (int i = 0; i < alternateNamesArray.length; i++) {
            String alternateSTR = alternateNamesArray[i].trim();
            alternateSTR = TP04Q04.shambles("'", "", alternateSTR);
        ...
        // alternate actors
        String tmp2 = TP04Q04.shambles("[", "", info[11]);
        tmp2 = TP04Q04.shambles("]", "", tmp2);
        String[] alternateActorsArray = TP04Q04.cutter(',', tmp2);
