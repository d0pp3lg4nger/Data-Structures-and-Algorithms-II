...
Personagem remover(Fila *fila)
{
    ...
    Personagem resp = fila->personagem[fila->primeiro];
...
int calcMedia(Fila *fila)
{
    ...
    for (i = fila->primeiro; i != fila->ultimo; i = ((i + 1) % TAM))
    {
        resp += fila->personagem[i].yearOfBirth;
        count++;
    }
    if (count == 0)
    {
...
int main(int argc, char const *argv[])
{
    ...
    printf("[ Head ]\n");
    int k = 0;
    for (int i = personagens->primeiro; i != personagens->ultimo; i = ((i + 1) % TAM))
    {
        print(getElementByID(getId(personagens, i), lista), k++);
    }
    printf("[ Tail ]\n");
