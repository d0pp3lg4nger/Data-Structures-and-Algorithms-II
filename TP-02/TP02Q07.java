...
╚══════╝╚══════╝   ╚═╝      ╚═╝   ╚══════╝╚═╝  ╚═╝╚══════╝    ╚═════╝       ╚═════╝ ╚══════╝   ╚═╝      ╚═╝   ╚══════╝╚═╝  ╚═╝╚══════╝
    ...
    public void read(String line) throws Exception {
        String[] info = TP02Q07.cutter(';', line);

        setId(info[0]);
        setName(info[1]);

        // alternate names
        String tmp = TP02Q07.shambles("[", "", info[2]);
        tmp = TP02Q07.shambles("]", "", tmp);
        String[] alternateNamesArray = TP02Q07.cutter(',', tmp);

        for (int i = 0; i < alternateNamesArray.length; i++) {
            String alternateSTR = alternateNamesArray[i].trim();
            alternateSTR = TP02Q07.shambles("'", "", alternateSTR);
        ...
        // alternate actors
        String tmp2 = TP02Q07.shambles("[", "", info[11]);
        tmp2 = TP02Q07.shambles("]", "", tmp2);
        String[] alternateActorsArray = TP02Q07.cutter(',', tmp2);
    ...
    public void print(Lista personagens, String id) {
        ...
        if (personagem.getDateOfBirth() != null) {
            ...
            String[] dateCut = TP02Q07.cutter('-', date);
            date = dateCut[2];
            date += "-";
            date += dateCut[1];
            date += "-";
...
public class TP02Q07 {
    ...
    public static void main(String[] args) {
        ...
        personagem.writeLog("matricula_insercao.txt", personagem.getCountComp(), personagem.getCountMov(), execTime);

        // ------------------ Escrever a saida ---------------
        int i = 0;
        for (Personagem p : personagem.getPersonagensID()) {
            if (p != null) {
                p.print(personagens, personagem.getPersonagensID()[i].getId());
            }
            i++;
        }
