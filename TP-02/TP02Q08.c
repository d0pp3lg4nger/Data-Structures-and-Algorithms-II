...
int shellSort(String *arr, int n)
{
    ...
    for (int gap = n / 2; gap > 0; gap /= 2)
    {
        ...
        {
            p1 = getElementByID(arr[i]);

            String temp = p1.id;
            String tmp = p1.eyeColour;

            int j;
            // comparar e desempatar por nome
            for (j = i; j >= gap && isBigger(getElementByID(arr[j - gap]).eyeColour, tmp, getElementByID(arr[j-gap]), p1); j -= gap)
            {
                countMov++;
...
int main(int argc, char const *argv[])
{
    ...
    // ------------ Escrever no arquivo --------------
    arq = fopen("matricula_shellsort.txt", "wt");
