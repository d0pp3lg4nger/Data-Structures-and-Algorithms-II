...
╚══════╝╚══════╝   ╚═╝      ╚═╝   ╚══════╝╚═╝  ╚═╝╚══════╝    ╚═════╝       ╚═════╝ ╚══════╝   ╚═╝      ╚═╝   ╚══════╝╚═╝  ╚═╝╚══════╝
    ...
    public void read(String line) throws Exception {
        String[] info = TP02Q05.cutter(';', line);

        setId(info[0]);
        setName(info[1]);

        // alternate names
        String tmp = TP02Q05.shambles("[", "", info[2]);
        tmp = TP02Q05.shambles("]", "", tmp);
        String[] alternateNamesArray = TP02Q05.cutter(',', tmp);

        for (int i = 0; i < alternateNamesArray.length; i++) {
            String alternateSTR = alternateNamesArray[i].trim();
            alternateSTR = TP02Q05.shambles("'", "", alternateSTR);
        ...
        // alternate actors
        String tmp2 = TP02Q05.shambles("[", "", info[11]);
        tmp2 = TP02Q05.shambles("]", "", tmp2);
        String[] alternateActorsArray = TP02Q05.cutter(',', tmp2);
    ...
    public void print(Lista personagens, String id) {
        ...
        if (personagem.getDateOfBirth() != null) {
            ...
            String[] dateCut = TP02Q05.cutter('-', date);
            date = dateCut[2];
            date += "-";
            date += dateCut[1];
            date += "-";
    ...
    // ------------- Pesquisa Sequencial ---------------------
    public void selection(String[] id, Lista personagens, String name) {
        setCountComp(0);
        setCountMov(0);
        long startTime = System.nanoTime(); // iniciar tempo de execucao
        for (int x = 0; id[x] != null; x++) {
            int smaller = x;
            for (int i = (x + 1); id[i] != null; i++) {
                Personagem personagem1 = personagens.getCharacterByID(id[smaller]);
                Personagem personagem2 = personagens.getCharacterByID(id[i]);
                if (isSmaller(personagem1.name, personagem2.name)) {
                    smaller = i;
                }
            }
            countMov += 3;
        ...
        long endTime = System.nanoTime(); // finalizar tempo
        long execTime = endTime - startTime; // calcular o tempo
        double msExecTime = execTime/1000000.0;
        writeLog("matricula_selecao.txt", countComp, countMov, msExecTime); // escrever no arquivo
    }

    // --------------- Escrever aquivo log ---------------
    public static void writeLog(String fileName, int numComp, int numMov, double execTime) {
...
public class TP02Q05 {
    ...
    public static void main(String[] args) {
        ...
        personagem.selection(idArray, personagens, line);

        // ------------------ Escrever a saida ---------------
        for (int i = 0; idArray[i] != null; i++) {
            personagem.print(personagens, idArray[i]);
        }
