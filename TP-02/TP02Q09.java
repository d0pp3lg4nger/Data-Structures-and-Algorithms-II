...
╚══════╝╚══════╝   ╚═╝      ╚═╝   ╚══════╝╚═╝  ╚═╝╚══════╝    ╚═════╝       ╚═════╝ ╚══════╝   ╚═╝      ╚═╝   ╚══════╝╚═╝  ╚═╝╚══════╝
    ...
    public void read(String line) throws Exception {
        String[] info = TP02Q09.cutter(';', line);

        setId(info[0]);
        setName(info[1]);

        // alternate names
        String tmp = TP02Q09.shambles("[", "", info[2]);
        tmp = TP02Q09.shambles("]", "", tmp);
        String[] alternateNamesArray = TP02Q09.cutter(',', tmp);

        for (int i = 0; i < alternateNamesArray.length; i++) {
            String alternateSTR = alternateNamesArray[i].trim();
            alternateSTR = TP02Q09.shambles("'", "", alternateSTR);
        ...
        // alternate actors
        String tmp2 = TP02Q09.shambles("[", "", info[11]);
        tmp2 = TP02Q09.shambles("]", "", tmp2);
        String[] alternateActorsArray = TP02Q09.cutter(',', tmp2);
    ...
    public void print(Lista personagens, int i) {
        ...
        if (personagem.getDateOfBirth() != null) {
            ...
            String[] dateCut = TP02Q09.cutter('-', date);
            date = dateCut[2];
            date += "-";
            date += dateCut[1];
            date += "-";
    ...
    public void heapsort(Lista lista) {
        ...
        for (int tamHeap = 2; tamHeap <= n; tamHeap++) {
            construir(tamHeap, lista.getCharacterByID(idArray[tamHeap]), lista.getCharacterByID(idArray[tamHeap/2]), lista);
        }
        ...
        while (tamHeap > 1) {
            swap(1, tamHeap--);
            reconstruir(tamHeap, lista);
        }

        // voltar para posicao zero
        countMov++;
        tmp = idArray;
        idArray = new String[n];
        for (int i = 0; i < n; i++) {
            countMov++;
            idArray[i] = tmp[i+1];
        }
    ...
    public void construir(int tamHeap, Personagem pEnd, Personagem p, Lista l) {
        for (int i = tamHeap; (i > 1) && (isBigger(l.getCharacterByID(idArray[i]).hairColour,
        l.getCharacterByID(idArray[i/2]).hairColour, l.getCharacterByID(idArray[i]),
        l.getCharacterByID(idArray[i/2]))); i /= 2) {
            swap(i, i / 2);
        }
    }

    public void reconstruir(int tamHeap, Lista l) {
        int i = 1;
        while (i <= (tamHeap / 2)) {
            int filho = getMaiorFilho(i, tamHeap, l);
            if (isBigger(l.getCharacterByID(idArray[filho]).hairColour, 
            l.getCharacterByID(idArray[i]).hairColour, l.getCharacterByID(idArray[filho]), 
            l.getCharacterByID(idArray[i]))) {
                swap(i, filho);
                i = filho;
            } else {
                i = tamHeap;
    ...
    public int getMaiorFilho(int i, int tamHeap, Lista l) {
        int filho;
        if (2 * i == tamHeap || isBigger(l.getCharacterByID(idArray[2 * i]).hairColour,
                l.getCharacterByID(idArray[2 * i + 1]).hairColour, l.getCharacterByID(idArray[2 * i]),
                l.getCharacterByID(idArray[2 * i + 1]))) {
            filho = 2 * i;
        } else {
            filho = 2 * i + 1;
        }
...
public class TP02Q09 {
    ...
    public static void main(String[] args) {
        ...
        personagem.heapsort(personagens);
