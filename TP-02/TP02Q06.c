...
// ------------ Selecao --------------------
void selection(String* idArray, int n, int i){
    if (i < n-1)
    {
        int smaller = i;
        for(int j = (i+1); j < n; j++){
            Personagem personagem1 = getElementByID(idArray[smaller]);
            Personagem personagem2 = getElementByID(idArray[j]);
            if (isBigger(personagem2.name, personagem1.name))
            {
                smaller = j;
            }
        }
        countMov += 3;
        swap(idArray, i, smaller);
        selection(idArray, n, i+1); // chamada recursiva
    }
}
...
int main(int argc, char const *argv[])
{
    ...
    // ------------ Escrever no arquivo --------------
    arq = fopen("matricula_selecaoRecursiva.txt", "wt");
    fprintf(arq, "815373\t %d \t %d \t%fms", countComp, countMov, execTime);
