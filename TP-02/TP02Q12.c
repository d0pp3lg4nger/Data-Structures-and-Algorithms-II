...
// ------------ bolha --------------------
void bolha(String *arr, int n)
{
    for (size_t i = 0; i < n - 1; i++)
    {
        for (size_t j = 0; j < n - i - 1; j++)
        {
            if (isBigger(getElementByID(arr[j]).hairColour, getElementByID(arr[j+1]).hairColour,
            getElementByID(arr[j]), getElementByID(arr[j+1])))
            {
                swap(arr, j, j+1);
            }
        }
...
int main(int argc, char const *argv[])
{
    ...
    // ------------ Escrever no arquivo --------------
    arq = fopen("matricula_bolha.txt", "wt");
